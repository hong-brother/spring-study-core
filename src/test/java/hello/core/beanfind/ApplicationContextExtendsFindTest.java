...
public class ApplicationContextExtendsFindTest {
    ...
    @Test
    @DisplayName("부모 타입으로 조회시, 자식이 둘 이싱 있으면, 중복 오류가 발생한다.")
    ...
    @Test
    @DisplayName("부모 타입으로 조회시, 자식이 둘 이싱 있으면, 빈 이름을 지정하면 된다.")
    ...
    void findBeanBySubType() {
        DiscountPolicy discountPolicy = applicationContext.getBean(RateDiscountPolicy.class);
        assertThat(discountPolicy).isInstanceOf(RateDiscountPolicy.class);
    }

    @Test
    @DisplayName("부포 타입으로 모두 조회하기")
    ...
    void findAllBeanByObjectType() {
        ...
        System.out.println("all count = " + beansOfType.size());
    }
